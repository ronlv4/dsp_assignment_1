...
To run our code you just need to ron localapp.jar in the following way:
...
This file neeeds to contain access key id, secret access key and session token.
for example: 
[default]
...
The local app initializes one manager ec2 container (if its not running), writes the input file to pre made S3 bucket and a message to pre made SQS queue.
...
The manager gets the message from the sqs queue and immideatly delegates the work to another thread (from the thread pool) so he can keep listening for new requests.
...
Then, it places an sqs message per every line in the worker queue. The message includes the file name and analysis time, and also the queue name in which it will wait for response.
...
Then, it waits for responses from the workers, waiting till it gets all of the unique order properties.
...
If there is a termination property, the Manager will wait until all workers are done, then it will send n termination messages in the sqs qeueue.
...
On receiving termination message, the worker finishes parsing and terminates gracefully including it’s own EC2 instance.
...
- We are not storing credentials in our code at all, we are using the default credntial chain provider and the program is looking for a credentials file in the users computer.
- The program will work with any number of users, the Manager always remains available. Our bottlenecks are the threadpool in the manager and the workers themselves.
  If the workers will work fast enough, the threadpool will be able to keep up with the load.
...
- In the termination process, we block any further requests from local apps.
  Then we wait for all of the threads to be done (means all of the local apps are taken care of).
  Then the manager sends termination messages to all workers, which terminate gracefully, and finally the manager terminates.
- All of the workers are working (when there is enough work). When there are less messages than workers some will be idle.
- As mentioned before, the threads in the manager (not the manger itself) are waiting for the sqs messages from the workers.
  This could be a problem is certain scenerios (when the workers are slow), in this case the base solution would be using one thread to wait for answers and create runnables to process the answers.
